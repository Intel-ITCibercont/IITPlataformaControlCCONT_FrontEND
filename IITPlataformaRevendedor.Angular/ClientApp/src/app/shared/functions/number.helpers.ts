...
export const agregarCeros = (num: number, totalLength: number): string  => {
    return String(num).padStart(totalLength, '0');
  }

export const numeroConFormato = (valor: any, precision: number = 3) => {
    try {
...
export const esNumero = (cadena: string) => {
    ...
    let array = cadena.toString().trim().split('');
...
export const redondearNumero = (num, length): string => {
