...
import { EnviromentUrlService } from '../services/enviroment-url.service';
import { AuthService } from '../services/auth.service';

@Injectable({
  providedIn: 'root'
})
export class TokenFacturacionInterceptor implements HttpInterceptor {

  constructor(
    private localstorageService: LocalStorageService,
    private authService: AuthService,
    private enviromentUrlService: EnviromentUrlService
  ) {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    const request = this.addHeadersFacturadorComercial(req);

    return next.handle(request);
  }

  addHeadersFacturadorComercial = (req: HttpRequest<any>): HttpRequest<any> => {

    if (req.url.toString().includes(this.enviromentUrlService.urlAddress)) {
